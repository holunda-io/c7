...
class RuntimeServiceVariablesJavaSerializationITest : CamundaRestClientITestBase<RuntimeService, RuntimeServiceActionStage, RuntimeServiceAssertStage>() {
  ...
  fun `should add new, update and delete existing variables`() {
    ...
    THEN
      .process_instance_exists(processDefinitionKey) { instance, stage ->
        ...
        assertThat(stage.remoteService.getVariables(instance.id)).containsKeys("VAR1", "VAR3", "VAR4", "VAR5", "VAR6")
        assertThat(stage.remoteService.getVariable(instance.id, "VAR1")).isEqualTo("NEW VALUE")
        assertThat(stage.remoteService.getVariable(instance.id, "VAR3")).isEqualTo("VAL3")
        assertThat(stage.remoteService.getVariable(instance.id, "VAR4")).isEqualTo("My object value")
        assertThat(stage.remoteService.getVariable(instance.id, "VAR5")).isEqualTo("untyped")
        assertThat(stage.remoteService.getVariableTyped<StringValue>(instance.id, "VAR6")).isEqualTo(stringValue("typed"))

        assertThat(stage.remoteService.getVariable(instance.id, "VAR1")).isEqualTo(
          stage.remoteService.getVariable(instance.id, "VAR1")
        )
        assertThat(stage.remoteService.getVariable(instance.id, "VAR7")).isEqualTo(
          stage.remoteService.getVariable(instance.id, "VAR7")
        )
  ...
  fun `serialize and deserialize bigdecimal as java serialization`() {
    ...
    WHEN
      .remoteService.setVariable(GIVEN.processInstance.id, "BD1", BigDecimal.ONE)
    THEN
      .process_instance_exists(processDefinitionKey) { instance, stage ->
        assertThat(instance.businessKey).isEqualTo("my-business-key1")
        assertThat(instance.caseInstanceId).isEqualTo("caseInstanceId1")

        assertThat(stage.remoteService.getVariable(instance.id, "BD1")).isEqualTo(BigDecimal.ONE)
      }
  }
