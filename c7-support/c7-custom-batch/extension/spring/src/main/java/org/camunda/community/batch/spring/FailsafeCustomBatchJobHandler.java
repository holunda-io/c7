...
/**
 * Job handler that encapsulates the call to {@link #executeFailsafe(Serializable, CommandContext)}
 ...
 * occurs.
 * <p>
 * By default, any exception risen is ignored, this behavior can be overwritten via {@link #handleRuntimeException(RuntimeException)},
...
public abstract class FailsafeCustomBatchJobHandler<T extends Serializable> extends CustomBatchJobHandler<T> {
  ...
  private final static Properties TRANSACTION_ATTRIBUTES = new Properties() {{
   ...
   * replaced with some central implementation.
   *
   * @param target the wrapped instance
   * @param <P>    wrapped type
   * @return proxy
   */
  protected <P> P requireNewTransaction(P target) {
